...
def predict_timber(weights_yolov5=WEIGHTS_YOLOV5, weights_class=WEIGHTS_CLASS, img_dir=IMG_DIR, path=DETECT_DIR, conf=CONF, bbox_type='ellipse',
                   final_wide=800, show=1):
    ...
    img_edited.save(detect_dir + f'/{s_overall}_{text_arr}.png')  # change save_dir later!
    ...
    df.to_csv(detect_dir + f'/{s_overall}_{text_arr}.csv')  # change save_dir later!

    return resized, bboxes, plate, text_arr, df, img_edited
